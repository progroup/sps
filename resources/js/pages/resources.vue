...
export default defineComponent({
  setup() {
    ...
    onMounted(async () => {
      const response = await fetch('https://api.victortolbert.com/resources')
...
<template>
  <section v-if="resources.length" class="w-full max-w-3xl mt-8 space-y-8 lg:max-w-7xl">
    <div class="flex w-full" v-for="resource in resources" :key="resource.id">
      <div class="flex flex-col items-center flex-shrink-0 w-16 pt-2 mr-4 space-y-1">
        <svg
          width="24"
          height="24"
      ...
      <div class="w-full">
        ...
        <div class="w-full mt-1">
          ...
          <div class="flex items-center w-full mt-3 space-x-4 text-xs">
            <div>
              <span class="text-xs font-medium tracking-wide uppercase">Published</span>
              {{ formatDate(resource.published_at) }}
            </div>
            <div>
              <span class="text-xs font-medium tracking-wide uppercase">Last Modified</span>
              {{ formatDate(resource.modified_at) }}
            </div>
            <div v-if="false">
              <span class="text-xs font-medium tracking-wide uppercase">Author</span>
              {{ resource.author }}
            </div>
          </div>
        </div>
